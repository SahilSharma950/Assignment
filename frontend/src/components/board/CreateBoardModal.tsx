...
export const CreateBoardModal: FC<CreateBoardModalProps> = ({ isOpen, workspaceId, onClose, onCreated }) => {
  const {
    register,
    handleSubmit,
    reset,
  ...
  const onSubmit = async (values: CreateBoardValues) => {
    try {
      const response = await boardApi.create({ ...values, workspaceId });
      onCreated(response.data);
      onClose();
    } catch (err: any) {
      setError('root', {
        message: err.response?.data?.message || 'Failed to create board. Please try again.',
      });
    }
  };
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-surface-900 border border-slate-200 dark:border-surface-700 rounded-2xl shadow-2xl p-8 animate-fade-in">
        <div className="flex items-center justify-between mb-7">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white">New Board</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
          <div className="space-y-2">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300" htmlFor="board-name">
              Name
            </label>
            <input
              id="board-name"
          ...
          <div className="space-y-2">
            <label className="block text-sm font-medium text-slate-700 dark:text-slate-300" htmlFor="board-description">
